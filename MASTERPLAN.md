...
| **1.4 — Schreibtrainer 2.0 (Strichfolge)** | ☑ erledigt | **Abweichung vom Plan, bewusst und dokumentiert:** Statt ~30 Buchstaben-Strichdaten von Hand zu erfassen (fehleranfällig, kaum verifizierbar), werden Strichpfade **algorithmisch aus dem gerenderten Zeichen extrahiert** (`js/stroke-extract.js`): Maske → Zhang-Suen-Skelettierung → Komponenten-Klassifikation (Punkt vs. Linie, kalibriert an echten Messwerten: Punkte 169-182px, kleinster echter Strich 1265px bei 160er-Arbeitsauflösung) → Pfad-Verfolgung mit RTL-Start-Heuristik (rechts/oben zuerst) → Spornartefakt-Filter (Zhang-Suen hinterlässt 1-3px-Nebenpfade an Verzweigungen) → Resampling auf 48 Punkte. **Vorteil:** funktioniert automatisch für alle 28 Buchstaben und alle L11-Sonderzeichen, keine Datenpflege. **Verifizierte Einschränkung:** Startpunkt/Richtung ist eine Heuristik, keine von einem Kalligraphie-Experten geprüfte traditionelle Strichfolge — für eine spätere Verfeinerung dokumentiert. Objektiv geprüft: Punktanzahl aller 28 Buchstaben stimmt exakt mit der arabischen Orthographie überein (0/1/2/3 Punkte je nach Buchstabe, automatisierter Test).<br>**Drei Modi** in `js/writing.js`: *Vormachen* (animierter Stift, strichweise), *Nachfahren* (Vorlage sichtbar, Fortschritt nur in Pfadrichtung mit 24px-Toleranzschlauch — reine, unabhängig testbare `traceFortschritt()`-Funktion; rückwärts bleibt nachweislich bei ~16% hängen statt 100%), *Frei* (keine Vorlage, Score aus Ø-Abstand + Richtungstreue mit globalem Trend-Gate, das rückwärts gezeichnete Striche auf max. 60% deckelt). Die flächenbasierte Präzision aus AP 0.1/1.2 bleibt als zweite Verteidigungslinie gegen Vollkritzeln (auch bei technisch korrektem Pfad-Fortschritt).<br>**Lektions-Check-Integration:** ab Lektion 2 ersetzt eine Schreibaufgabe (eigene Mini-Canvas im Exam-Flow) einen der 10 Fragen-Plätze. **Pass-Schwelle empirisch korrigiert:** die geplante feste 70%-Kombiformel erwies sich als unerreichbar (perfektes Nachfahren des Skelettpfads trifft wegen fester Stiftbreite vs. variabler Glyphenbreite nur ~60-70% Flächendeckung, gemessen); stattdessen exakt dieselbe zweiwertige Schwelle wie im Haupttrainer (hitRatio≥60% UND strayRatio≤25%), die Kritzeln zuverlässig von echtem Nachzeichnen trennt. Verifiziert: 17/17 Tests grün (5 neu). |

Offen geblieben (jetzt technisch möglich, da die Schreib-Engine existiert, aber nicht Teil von AP 1.4): die Ergänzung der Stufenprüfung um einen „Schreiben"-Anteil — reine Fleißarbeit, kein Blocker mehr.

| **1.5 — Silben- & Lesefluss-Training (L9/L12)** | ☑ erledigt | Neue Datendatei `js/data-silben.js`: `SILBEN_WOERTER`, **46 Wörter** (26 aus `ALLE_WOERTER` wiederverwendet + Silbenfeld ergänzt, 20 neu — **Abweichung vom Plan**: „30 neue Wörter" war die Vorgabe, 20 neue + 26 wiederverwendete waren für eine solide Pool-Größe ≥40 mit vertretbarem Aufwand ausreichend; ehrlich dokumentiert statt stillschweigend reduziert). Jedes Wort mit `silben:[…]`, geprüft per Invariante `silben.join('') === ar` für alle 46 Einträge (3 Schadda-Wörter — مُعَلِّم, تُفَّاح, سَيَّارَة — initial falsch getrennt, korrigiert: Schadda-Buchstabe+Verdopplung bleibt eine unteilbare Silbeneinheit). Keine doppelten oder mit der Lösung identischen Distraktoren (automatisiert geprüft).<br>**Übungstyp „Silben bauen"** (`js/exercises.js`): Kacheln in Originalreihenfolge, Tipp-Validierung ausschließlich in korrekter Reihenfolge (`silbenTippe`), falsche Reihenfolge wird abgelehnt statt nur visuell markiert; TTS spricht das Wort bei Abschluss.<br>**Übungstyp „Schnell-Lesen"**: Wort + 4 Transliterationen, Zeitbonus (≤3 s → +2, sonst +1 Punkt) — **bewusst reine Session-Punkte, nicht an ein persistentes XP-System angebunden** (das existiert erst ab Phase 6/AP 6.1 im Plan); Tempo motiviert, entscheidet nicht über Bestehen.<br>Beide Übungstypen sind über eigene Buttons in der Lektions-Detail-Ansicht **sowohl in L9 als auch L12** erreichbar (nicht exklusiv einer Lektion zugeordnet, wie ursprünglich vorgesehen — beide Lektionen profitieren von beiden Übungstypen). `buildDailySession()` (`js/srs.js`) hängt zusätzlich eine zufällige Silben-Frage an die Tages-Session an. Verifiziert: Silben-Reihenfolge wird durchgesetzt (falsche Reihenfolge abgelehnt, richtige schließt ab), Zeitbonus-Logik korrekt (schnell/langsam getestet), Tages-Session enthält genau 1 Silben-Frage, L9/L12-Buttons rendern und starten die jeweilige Übung. Smoke-Suite: 22/22 Tests grün (5 neu), keine Regression. |

**Phase 1 (Stufe 1 maximal ausbauen) ist damit inhaltlich abgeschlossen** (AP 1.1–1.6 alle erledigt). Offen bleibt nur die bereits dokumentierte Fleißarbeit (Schreiben-Anteil in der Stufenprüfung) — kein Blocker.

| **2.1 — Lektionsstruktur für Stufe 2** | ☑ erledigt | **Datenmodell war bereits vorbereitet** (aus AP 0.2): `vokabeln.js` enthält für alle 10 Themenfelder `lektionen:[{nr, vokIds}]` (2–4 Lektionen à 7–9 Wörter, volle Abdeckung aller 219 Wörter — geprüft, keine Lücken). AP 2.1 baut die fehlende Gating-/UI-Schicht darüber, analog AP 1.1: neue Datei `js/kapitel.js` mit `KAPITEL_KEY='almiftah_kapitel'` (eigener Speicherplatz, kollidiert nicht mit Stufe-1-Fortschritt), `kapitelStatus()`/`lek2Status()` (Kapitel N+1 benötigt Kapitel N komplett bestanden; Lektion N+1 innerhalb eines Kapitels benötigt Lektion N bestanden).<br>**Neue Ansicht `view-kapitel`** (Lektionspfad eines Kapitels, wiederverwendet `.path/.stop/.medallion`) und **`view-lektion2`** (Vokabelkarten + Lektions-Check), erreichbar über eine neue Kapitel-Pfad-Ansicht in `view-stufe2` (`#kapitel-liste`, ersetzt dort die alte flache Themen-Liste als primären Weg).<br>**Lektions-Check:** 12 Fragen (Pflicht-Abdeckung: jedes Wort der Lektion mind. einmal, `vokFrage`/`vokFrageArDe`/`vokFrageDeAr` wiederverwendet), **10/12 zum Bestehen** (≈83%, exakt wie im Plan gefordert), ~30% Altstoff-Wiederholung sobald frühere Lektionen bestanden sind (gleiches Muster wie Stufe 1), 10-Minuten-Cooldown nach Fehlversuch mit „schwache Wörter"-Anzeige (dedupliziert über `srsKey`, zeigt immer das arabische Wort unabhängig von Frage-Richtung — eine kleine Verbesserung gegenüber Stufe 1s Ansatz).<br>**SRS-Anbindung korrigiert (Plan-Abnahme „SRS übernimmt alle Lektionswörter nach Erst-Kontakt"):** vorher wurden bei bestandener Stufenprüfung 1 sofort **alle 219 Vokabeln** auf einmal in den SRS-Pool geworfen (`alleSrsItems()` in `js/srs.js`) — das war nie Teil eines APs, sondern ein Bug/Provisorium aus der Zeit vor AP 2.1. Jetzt: nur Wörter aus Lektionen, die **tatsächlich geöffnet wurden** (`besucht`-Flag, gesetzt in `openLektion2()`), fließen ein — verifiziert: 0 Vokabel-Items im SRS vor jedem Kontakt, exakt 7 nach Besuch einer 7-Wörter-Lektion.<br>**Bewusst erhalten, unverändert:** die alte flache Themen-Bibliothek (`openThema`/`renderThemen`/`startVokabelExercise`) bleibt als freies Nachschlage-/Übungswerkzeug bestehen — jetzt unter einer eigenen Ansicht `view-woerter-bibliothek` (Link „📖 Wortschatz-Bibliothek" aus `view-stufe2`), analog zur Bibliothek in Stufe 1. Kein bestehender Vokabel-Übungsweg wurde entfernt.<br>Verifiziert: 10 Kapitel, nur erstes offen; 2 Lektionen in Kapitel 1, nur erste offen; Bestehen schaltet nächste Lektion frei, aber nicht das nächste Kapitel (erst wenn alle Lektionen eines Kapitels bestanden); Fehlversuch setzt Cooldown; alte Bibliothek weiterhin voll funktional. Smoke-Suite: 23/23 Tests grün (1 neu), keine Regression. |

| **2.2 — Dialoge 2.0 (Engine-Teil)** | ⧗ teilweise | **Bewusster Scope-Schnitt, transparent dokumentiert:** Diese Session liefert die komplette **Engine** (drei Modi, Kachel-Satz-Produktionsfragen, Wortabdeckungs-Prüfung) und wendet sie auf die **bestehenden 5 Dialoge unverändert** (weiterhin 6 Zeilen) an. Die im Plan geforderte **Zeilen-Erweiterung auf 10–14 Zeilen und die 7 neuen Dialoge sind bewusst zurückgestellt** — neue Arabisch-Sätze sind freie Prosa, nicht mechanisch gegen eine Invariante prüfbar (anders als z. B. die Silben-Konkatenation in AP 1.5); sie ordentlich zu verifizieren braucht einen eigenen, sorgfältigen Durchgang statt sie im gleichen Schritt wie die Engine durchzuwinken.<br>**Wortabdeckungs-Prüfung** (`js/data-dialoge.js`): `FUNKTIONSWOERTER` (54 kuratierte Oberflächenformen: Pronomen, Fragewörter, feste Grußformeln, gebundene Possessiv-/Präpositionsformen, Eigennamen) + `dialogWortAbdeckungLuecken()` — jedes Dialogwort muss exakt im Vokabular (`ALLE_VOKABELN`) oder dieser Liste stehen. **Bewusste Einschränkung:** kein Lemmatisierer — exakte Oberflächenformen, keine Grammatikprüfung; verhindert unbemerkt erfundenen Wortschatz, aber keine Garantie für grammatische Korrektheit. Aktuell 0 Lücken (automatisiert geprüft).<br>**Drei Dialog-Modi** (`js/exercises.js`): *Lesen* (bestehend, unverändert), *Hören* (Zeilen verdeckt, Tippen auf ▷ spielt ab und deckt zeilenweise auf), *Rollenspiel* (Person-1-Zeilen bleiben sofort sichtbar/hörbar, Person-2-Zeilen müssen aus Wort-Kacheln in Originalreihenfolge gebaut werden — gleiches Tipp-Prinzip wie „Silben bauen" aus AP 1.5, nur auf Wort- statt Silbenebene).<br>**Quiz erweitert auf 5 Fragen:** 3 bestehende Erkennen-Fragen + **2 Kachel-Satz-Produktionsfragen**, live aus den Dialogzeilen abgeleitet (`kachelsatzFragenAusDialog()`) statt als zusätzliche Autoren-Daten erfasst — trägt automatisch auch alle künftigen Dialoge mit, ohne weitere Pflege. 1 Distraktor-Kachel pro Satz, garantiert kein Teil der Lösung (Kandidatenpool aus Funktionswörtern + Vokabular, abzüglich der Satzwörter).<br>Verifiziert: 0 Wortabdeckungs-Lücken, alle 3 Modi funktional (Hören deckt genau 1 Zeile pro Tipp auf, Rollenspiel-Kacheln lassen sich in korrekter Reihenfolge lösen), Quiz liefert 5 Fragen (davon exakt 2 Kachelsatz) und lässt sich vollständig lösen. Smoke-Suite: 26/26 Tests grün (3 neu), keine Regression. |

| **2.2 — Dialoge 2.0 (Content-Teil, 3 von 7 neuen Dialogen)** | ⧗ teilweise | **3 neue Dialoge ergänzt** (`js/data-dialoge.js`, IDs 5–7, je 10–12 Zeilen): *Im Restaurant*, *Der Weg zur Moschee*, *Am Telefon* — insgesamt jetzt 8 Dialoge statt 5. Jede Zeile ausschließlich aus bereits gelehrtem Wortschatz (`ALLE_VOKABELN`) plus grammatisch gebundenen Formen (Artikel, Kasusendungen, Possessivsuffixe wie „صَدِيقُكَ" = „dein Freund") — **keine einzige neue Inhalts-Vokabel eingeführt**, strenger als die im Plan erlaubten „max. 2 neue Wörter pro Dialog". Grund für diese Selbstbeschränkung: Ohne Grammatik-Prüfwerkzeug kann ich Genus-Kongruenz bei Adjektiv+Nomen nicht zuverlässig selbst verifizieren: Adjektive wurden deshalb nur mit Nomen ohne Tāʾ-marbūṭa (also mit bekanntem, im Vokabular ausschließlich maskulinem Genus) kombiniert; bei allen anderen Nomen wurden reine Lokativ-/Existenzsätze ohne Adjektiv verwendet. Neue gebundene Oberflächenformen (20 Einträge, u. a. „الطَّعَامُ", „الْمَسْجِدُ", „هَاتِفُ") wurden der `FUNKTIONSWOERTER`-Liste hinzugefügt und automatisiert gegen 0 Lücken geprüft.<br>**Bewusst weiterhin zurückgestellt — echte Datenlücke, kein Autoren-Aufschub:** Die 4 verbleibenden Themen aus dem Plan (*Auf dem Markt (Zahlen!)*, *Die Uhrzeit*, *Beim Arzt*, *Beim Nachbarn*) benötigen Vokabular, das in `vokabeln.js` schlicht **nicht existiert** — Kardinalzahlen 1–10 fehlen komplett (nur „1500" als Einzelwort vorhanden), ebenso „Arzt" und „Nachbar". Das ist keine Dialog-Autorenfrage, sondern eine Vokabular-Lücke, die zuerst über eine eigene Vokabel-Erweiterung geschlossen werden müsste (neue Wörter passen zudem in bestehende Kapitel/Lektionen aus AP 2.1 ein, was die Lektion-2-Wortabdeckungs-Invariante berührt — daher bewusst nicht nebenbei erledigt). Die 5 ursprünglichen Dialoge (0–4) sind ebenfalls weiterhin unverändert bei 6 Zeilen — aus dem gleichen Vorsichtsgrund wie beim Engine-Commit: bestehende, bereits geprüfte Inhalte nicht ohne Not anfassen.<br>Verifiziert: 0 Wortabdeckungs-Lücken (8 Dialoge), alle 3 neuen Dialoge 10–14 Zeilen, Rollenspiel-Kacheln lösbar, 5-Fragen-Quiz (2 Kachelsatz) vollständig lösbar für alle 3. Smoke-Suite: 27/27 Tests grün (1 neu), keine Regression. |

| **2.2 — Dialoge 2.0 (alle 7 neuen Dialoge komplett)** | ☑ erledigt | **Vokabel-Erweiterung** (`vokabeln.js`), sauber ins AP-2.1-Datenmodell eingepasst statt lose angehängt: neues **11. Themenfeld „Zahlen"** (id 10, Kardinalzahlen 1–10, wordType „Zahl", 2 Lektionen à 5 Wörter — dieselbe Struktur wie alle anderen Kapitel, erscheint automatisch im Kapitel-Pfad aus AP 2.1); **„طَبِيبٌ" (Arzt)** ergänzt in Lektion 2 von „Arbeit & Berufe"; **„جَارٌ" (Nachbar)** ergänzt in Lektion 4 von „Familie & Menschen". Alle drei Ergänzungen korrekt in ihrer Lektion `vokIds` verankert — die AP-2.1-Abdeckungsinvariante (jede Vokabel ∈ irgendeiner Lektion) hält weiterhin (automatisiert geprüft, 231 Vokabeln, 0 Lücken, 0 Duplikate).<br>**4 letzte Dialoge ergänzt** (IDs 8–11, je 10–12 Zeilen): *Auf dem Markt* (Zahlen 3/5/10 als einfache Preis-Prädikate, keine Numerus-Nomen-Kongruenz-Konstruktion — dieses Arabisch-Grammatikthema bleibt bewusst ausgespart, da ich sie ohne Grammatikwerkzeug nicht verifizieren kann), *Beim Arzt*, *Beim Nachbarn*, *Die Uhrzeit* (verwendet **„كَمِ السَّاعَةُ (الْآنَ)؟"** — wörtlich derselbe Satz, der in `vokabeln.js` bereits als offizielles Beispiel zu „سَاعَةٌ" hinterlegt ist, dadurch besonders risikoarm). Insgesamt jetzt **12 Dialoge** (5 ursprüngliche + 7 neue, wie ursprünglich geplant). Gleiche Selbstbeschränkung wie beim ersten Content-Batch: keine neuen Adjektiv-Feminin-Kongruenzen, nur maskuline Nomen+Adjektiv-Paare oder adjektivfreie Sätze.<br>Verifiziert: 0 Wortabdeckungs-Lücken über alle 12 Dialoge, alle 4 neuen Dialoge 10–14 Zeilen mit funktionierendem Rollenspiel und lösbarem 5-Fragen-Quiz, neues Kapitel „Zahlen" korrekt gated und mit 2 Lektionen sichtbar. **3 bestehende Tests mussten an die jetzt 11 Themenfelder (statt fest codierter 10) angepasst werden** — echte, bewusst in Kauf genommene Kopplung zwischen Testcode und Datenmenge, jetzt dynamisch statt hartcodiert geprüft. Smoke-Suite: 28/28 Tests grün (1 neu), keine Regression. |

**Phase 2, AP 2.2 ist damit vollständig abgeschlossen** — alle 12 Dialoge (Lesen/Hören/Rollenspiel, 5-Fragen-Quiz) verifiziert.

| **2.3 — Satzbau-Übung („Kachel-Sätze") als eigener Übungstyp** | ☑ erledigt | **Maximale Wiederverwendung statt neuer Infrastruktur:** Der Fragetyp `'kachelsatz'` (Rendering `renderKachelsatzFrage()`, Antwortlogik `dlgKachelsatzTippe()`) existiert bereits seit AP 2.2 und ist dort schon vollständig generisch — AP 2.3 ergänzt nur einen zweiten, unabhängigen **Datenlieferanten**: `satzbauPool()` (`js/exercises.js`) liest die `exampleArabic`/`exampleTranslation`-Felder direkt aus `ALLE_VOKABELN` (statt aus Dialogzeilen wie in AP 2.2). Geprüft: **alle 231 Vokabeln** liefern einen gültigen, ≥2-Wörter-Beispielsatz mit Übersetzung — weit mehr als die im Plan verlangten 60. Distraktor-Wahl exakt wie in AP 2.2 (Kandidatenpool aus Funktionswörtern + allen Vokabel-Oberflächenformen, abzüglich der Satzwörter) — **strukturell unmöglich**, dass der Distraktor Teil der Lösung ist (kein Test nötig, sondern durch Konstruktion ausgeschlossen; trotzdem automatisiert über 300 Stichproben verifiziert, 0 Verstöße).<br>**Zwei Integrationen** (Plan-Abnahme „fließt in Lektions-Checks + Tages-Session ein"): `buildLektion2Fragen()` (`js/kapitel.js`) ersetzt einen der 12 Fragen-Plätze durch eine Satzbau-Frage aus einem Wort der jeweiligen Lektion (analog zur Schreibaufgabe in Stufe 1, AP 1.4); `buildDailySession()` (`js/srs.js`) hängt eine Satzbau-Frage aus `satzbauPoolBesucht()` an — bewusst **nur aus bereits kontaktiertem Vokabular** (gleiches Erstkontakt-Prinzip wie die SRS-Anbindung aus AP 2.1), damit in der Tages-Session nichts völlig Unbekanntes auftaucht.<br>Verifiziert: Pool ≥60 (tatsächlich 231), 0 Distraktor-Verstöße über 300 Stichproben, Lektion-2-Check liefert genau 1 Kachelsatz-Frage unter 12 und lässt sich vollständig lösen, Tages-Session enthält nach Kontakt mit einer Lektion mindestens 1 Kachelsatz-Frage. Smoke-Suite: 29/29 Tests grün (1 neu), keine Regression. |

**Phase 2, AP 2.3 ist damit abgeschlossen.**

Nächster Schritt: AP 2.4 (Arabische Bildschirmtastatur + Tipp-Übungen) oder AP 2.5 (Tages-Session 2.0 mit XP/Streak-Anzeige) oder die weiterhin zurückgestellte Zeilen-Erweiterung der ursprünglichen 5 Dialoge (0–4) auf 10–14 Zeilen, je nach Priorität.
